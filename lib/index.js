...
const url = process.env.LUGARES_URL || '35.227.46.47';
const port = process.env.LUGARES_PORT || '4005';
const entryPoint = process.env.LUGARES_ENTRY || 'lugares';
...
const url$1 = process.env.CITAS_URL || '35.227.46.47';
const port$1 = process.env.CITAS_PORT || '4002';
const entryPoint$1 = process.env.CITAS_ENTRY || 'citas';
...
const resolvers$1 = {
	Query: {
		...
		citaByPersonaId: (_, { id }) =>
			generalRequest(`${URL$1}/personas/${id}`, 'GET'),
	},
	Mutation: {
		createCita: (_, { cita }) =>
			generalRequest(`${URL$1}`, 'POST', cita),
		updateCita: (_, { id, cita }) =>
			generalRequest(`${URL$1}/${id}`, 'PUT', cita),
		deleteCita: (_, { id }) =>
			generalRequest(`${URL$1}/${id}`, 'DELETE')
	}
};

const url$2 = process.env.GUSTOS_URL || '35.227.46.47';
const port$2 = process.env.GUSTOS_PORT || '4006';
const entryPoint$2 = process.env.GUSTOS_ENTRY || '';
...
const resolvers$2 = {
	Query: {
		allCategories: (_) =>
			getRequest(`${URL$2}categories`, ''),
		categoryById: (_, { id }) =>
			generalRequest(`${URL$2}categories/${id}`, 'GET'),
		categoriesByName: (_, { name }) =>
			generalRequest(`${URL$2}categories?by_name=${name}`, 'GET'),
		allSubcategories: (_) =>
			getRequest(`${URL$2}subcategories`, ''),
		subcategoryById: (_, { id }) =>
			generalRequest(`${URL$2}subcategories/${id}`, 'GET'),
		subcategoriesByCategory: (_, { category_id }) =>
			generalRequest(`${URL$2}subcategories?by_category=${category_id}`, 'GET'),
		subcategoriesByName: (_, { name }) =>
			generalRequest(`${URL$2}subcategories?by_name=${name}`, 'GET'),
		allPleasures: (_) =>
			getRequest(`${URL$2}pleasures`, ''),
		pleasureById: (_, { id }) =>
			generalRequest(`${URL$2}pleasures/${id}`, 'GET'),
		pleasuresBySubcategory: (_, { subcategory_id }) =>
			generalRequest(`${URL$2}pleasures?by_subcategory=${subcategory_id}`, 'GET'),
		pleasureByUser: (_, { user_id }) =>		
			generalRequest(`${URL$2}pleasures?by_user=${user_id}`, 'GET'),		        
		pleasuresByCategory: (_, { category_id }) =>
		        generalRequest(`${URL$2}pleasures?by_category=${category_id}`, 'GET'),
		pleasuresByName: (_, { name }) =>
		        generalRequest(`${URL$2}pleasures?by_name=${name}`, 'GET')
	},
	Mutation: {
		createCategory: (_, { category }) =>
			generalRequest(`${URL$2}categories`, 'POST', category),
		updateCategory: (_, { id, category }) =>
			generalRequest(`${URL$2}categories/${id}`, 'PUT', category),
		deleteCategory: (_, { id }) =>
			generalRequest(`${URL$2}categories/${id}`, 'DELETE'),
		createSubcategory: (_, { subcategory }) =>
			generalRequest(`${URL$2}subcategories`, 'POST', subcategory),
		updateSubcategory: (_, { id, subcategory }) =>
			generalRequest(`${URL$2}subcategories/${id}`, 'PUT', subcategory),
		deleteSubcategory: (_, { id }) =>
			generalRequest(`${URL$2}subcategories/${id}`, 'DELETE'),
		createPleasure: (_, { pleasure }) =>
			generalRequest(`${URL$2}pleasures`, 'POST', pleasure),
		updatePleasure: (_, { id, pleasure }) =>
			generalRequest(`${URL$2}pleasures/${id}`, 'PUT', pleasure),
		deletePleasure: (_, { id }) =>
			generalRequest(`${URL$2}pleasures/${id}`, 'DELETE')
	}
};

const url$3 = process.env.USUARIOS_URL || 'localhost';//'users-ms';
const port$3 = process.env.USUARIOS_PORT || '3000';//'4001';
const entryPoint$3 = process.env.USUARIOS_ENTRY || 'users';
...
const resolvers$3 = {
	...
	Mutation: {
		createToken: (_, { auth }) =>
			generalRequest(`http://${url$3}:${port$3}/user_token`, 'POST', auth),
		/*createUser: (_, { user }) =>
...
const url$4 = process.env.MATCH_URL || '35.227.46.47';
const port$4 = process.env.MATCH_PORT || '4003';
const entryPoint$4 = process.env.MATCH_ENTRY || 'match';
...
const resolvers$4 = {
	...
	Mutation: {
		createMatch: (_, { match }) =>
			generalRequest(`${URL$4}/`, 'POST', match),
		filtrateListPossibles: (_, {id, listUsers }) =>
			generalRequest(`${URL$4}/posibles/${id}`, 'POST', listUsers)
	}
};
...
var graphQLSchema = graphqlTools.makeExecutableSchema({
	...
	resolvers: merge(
		...
		resolvers,resolvers$1,resolvers$3,resolvers$2, resolvers$4
	)
});
